package com.mobarok.roundprogressbar
...
/**
 * Copyright (C) 2024 Mobarok Hossain
 * https://mobarokop.github.io
 */
class RoundProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    var progressBarWidth: Float = resources.getDimension(R.dimen.default_progressbar_size)
        set(value) {
            field = value.dpToPx()
    ...
    var backgroundProgressBarWidth: Float = resources.getDimension(R.dimen.default_progressbar_size_background)
        set(value) {
            field = value.dpToPx()
    ...
    private val indeterminateModeRunnable = Runnable {
        if (indeterminateMode) {
            ...
            this@RoundProgressBar.progressDirectionIndeterminateMode = this@RoundProgressBar.progressDirectionIndeterminateMode.reverse()
            if (this@RoundProgressBar.progressDirectionIndeterminateMode.isToRight()) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        val attributes = context.theme.obtainStyledAttributes(attrs, R.styleable.RoundProgressBar, 0, 0)

        // Value
        progress = attributes.getFloat(R.styleable.RoundProgressBar_progress, progress)
        progressMax = attributes.getFloat(R.styleable.RoundProgressBar_progressMax, progressMax)

        // StrokeWidth
        progressBarWidth = attributes.getDimension(R.styleable.RoundProgressBar_progressBarWidth, progressBarWidth).pxToDp()
        backgroundProgressBarWidth = attributes.getDimension(R.styleable.RoundProgressBar_progressBarBackgroundWidth, backgroundProgressBarWidth).pxToDp()

        // Color
        progressBarColor = attributes.getInt(R.styleable.RoundProgressBar_progressBarColor, progressBarColor)
        attributes.getColor(R.styleable.RoundProgressBar_progressBarColorStart, 0)
            .also { if (it != 0) progressBarColorStart = it }
        attributes.getColor(R.styleable.RoundProgressBar_progressBarColorEnd, 0)
            .also { if (it != 0) progressBarColorEnd = it }
        progressBarColorDirection = attributes.getInteger(R.styleable.RoundProgressBar_progressBarColorDirection, progressBarColorDirection.value).toGradientDirection()
        backgroundProgressBarColor = attributes.getInt(R.styleable.RoundProgressBar_progressBarBackgroundColor, backgroundProgressBarColor)
        attributes.getColor(R.styleable.RoundProgressBar_progressBarBackgroundColorStart, 0)
            .also { if (it != 0) backgroundProgressBarColorStart = it }
        attributes.getColor(R.styleable.RoundProgressBar_progressBarBackgroundColorEnd, 0)
            .also { if (it != 0) backgroundProgressBarColorEnd = it }
        backgroundProgressBarColorDirection = attributes.getInteger(R.styleable.RoundProgressBar_progressBarBackgroundColorDirection, backgroundProgressBarColorDirection.value).toGradientDirection()

        // Progress Direction
        progressDirection = attributes.getInteger(R.styleable.RoundProgressBar_progressDirection, progressDirection.value).toProgressDirection()

        // Round Border
        roundBorder = attributes.getBoolean(R.styleable.RoundProgressBar_setRoundBorder, roundBorder)

        // Angle
        startAngle = attributes.getFloat(R.styleable.RoundProgressBar_progressStartAngle, 0f)

        // Indeterminate Mode
        indeterminateMode = attributes.getBoolean(R.styleable.RoundProgressBar_indeterminateMode, indeterminateMode)
